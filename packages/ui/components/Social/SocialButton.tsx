...
import { Color, Theme, useColor, useColorMode } from '@xstyled/styled-components';
...
const cssVarRegex = /var\((?<name>[^,]+),(?<color>[^)]+)\)/;

const TelegramSocialButton = ({ color }: { color: SystemProp<Color<Theme>, Theme> }) => {
  return (
    <g fill="none" fillRule="evenodd">
      <SocialGroupOutline
        color={color as SystemProp<Color<Theme>, Theme>}
        strokeWidth="3"
        cx="50"
        cy="50"
        r="43.75"
      ...
      <SocialGroupIcon
        ...
        color={'#fff' as SystemProp<Color<Theme>, Theme>}
        fillRule="nonzero"
      />
    </g>
  );
...
const GithubSocialButton = ({
  ...
  const bgValue = useColor('bg');
  const result = cssVarRegex.exec(bgValue!.toString());
  const [bgColor, setBgColor] = useState(result!.groups!.color);

  useEffect(() => {
    if (envDetect.isBrowser) {
      const actualColor = getComputedStyle(document.body).getPropertyValue(result!.groups!.name);
  ...
  return (
    <g fill="none" fillRule="evenodd">
      ...
      />
      <SocialGroupIcon
        color={bgColor as SystemProp<Color<Theme>, Theme>}
...
type SocialButtonProps = {
  ...
  href?: string;
} & React.ComponentProps<typeof SocialLink>;

const SocialButton = (props: SocialButtonProps): ReactElement => {
  const { variant, href, ...rest } = props;

  const textValue = useColor('text');
  const [colorMode] = useColorMode();
  const result = cssVarRegex.exec(textValue!.toString());
  const [textColor, setTextColor] = useState(result!.groups!.color);
  ...
  }, [result, colorMode, setTextColor]);

  const [size, color, SocialButtonContent] = ((): [
    number,
    string,
    React.FC<{ color: SystemProp<Color<Theme>, Theme> }>,
  ] => {
    switch (variant) {
      case 'github':
        return [600, textColor, GithubSocialButton];
      case 'telegram':
  ...
  return (
    <SocialLink {...rest} href={href} target="_blank" rel="noopener noreferrer">
      <SocialSvg viewBox={`0 0 ${size} ${size}`} xmlns="http://www.w3.org/2000/svg">
        <title>{variant}</title>
        <SocialButtonContent color={color as SystemProp<Color<Theme>, Theme>} />
      </SocialSvg>
    </SocialLink>
  );
};
