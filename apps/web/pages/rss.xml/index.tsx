...
import {
  rssXml_BlogQuery,
  rssXml_BlogQuery$data,
} from '../../__generated__/rssXml_BlogQuery.graphql';
import { initEnvironment } from '../../relay';

const blogQuery = graphql`
  query rssXml_BlogQuery($id: ID!) {
    blog(id: $id) {
      owner {
        id
      }
      postsConnection(first: 100, orderBy: { key: "createdAt", order: desc })
        @connection(key: "rssXml_blog_postsConnection") {
        edges {
          node {
            id
            owner {
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  const posts: NonNullable<rssXml_BlogQuery$data['blog']>['postsConnection']['edges'] =
  ...
  const feed = new Feed({
    ...
    updated: parseISO(posts[0].node.updatedAt),
    generator: 'vlepo',
    feedLinks: {
